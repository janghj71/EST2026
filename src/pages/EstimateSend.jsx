...
export default function EstimateSend() {
  ...
  const { loading: fetching, fetchEstimateList } = useEstimateList();
  ...
  const onQuery = async () => {
    try {
      const dataset = await fetchEstimateList({ day1: outFrom, day2: outTo });
      setRows(dataset);
    ...
    await withLoading(async () => {
      ...
      for (const row of checkedRows) {
        ...
        try {
          ...
          await sendSms({ comcode, est_serial: row.est_serial, hp, callback: sendNo, smskind: "03", smstxt: templateResult.text || "" });
          try { await updateEstPrint(row.est_serial); } catch { /* empty */ }
          ok++;
        } catch { fail++; }
      }
    ...
    await withLoading(async () => {
      ...
      for (const row of checkedRows) {
        ...
        try {
          ...
          const effectiveSmskind = altkindcode || "03";
      ...
      for (const row of checkedRows) {
        ...
        try {
          const res = await sendEstimateMail({
            comcode, est_serial: row.est_serial, mailkind: "03C",
            mail_addr: mailAddr,
            mail_subject: row.carno ? `차량번호 ${row.carno} 점검정비견적서입니다` : "점검정비견적서입니다",
            mail_text: "",
          });
  ...
  const columns = useMemo(() => [
    ...
    { key: "inday", title: "입고일자", width: "9%", align: "left", render: (v) => v || "" },
    ...
    <div className="bg-zinc-50 flex flex-col min-h-0 h-full overflow-hidden">
      ...
      <div className="sticky top-0 z-20 border-b bg-white/90 backdrop-blur">
        <div className="app-container py-3">
          <div className="text-lg font-semibold text-zinc-900">견적서 문자 발송</div>
          <div className="text-xs text-zinc-500">입고일자 기준, 견적종결 건만 조회됩니다</div>
        </div>
      </div>
      ...
      <div className="app-container py-4 flex-1 min-h-0 overflow-hidden flex flex-col gap-3">
        ...
        <div className="flex items-center gap-3 bg-white rounded-md border border-zinc-200 px-4 py-3 flex-wrap">
          <span className="text-sm font-semibold text-zinc-700 shrink-0">입고일자</span>
          <input
            type="date"
            value={outFrom}
        ...
        <div className="flex-1 min-h-0 flex gap-3 overflow-hidden">
          ...
          <div className="w-[300px] shrink-0 flex flex-col rounded-md border border-zinc-200 bg-white overflow-hidden">
            <div className="px-3 py-2 border-b border-zinc-100 bg-zinc-50 shrink-0">
              <div className="text-sm font-semibold text-zinc-800">견적서 템플릿</div>
            </div>
            <div className="flex-1 overflow-auto p-3">
              {templateText ? (
                ...
                <div className="text-xs text-zinc-400 text-center mt-8">
                  템플릿을 불러오는 중이거나<br />등록된 견적서 템플릿이 없습니다
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col rounded-md border border-zinc-200 bg-white overflow-hidden">
            ...
            <div className="flex-1 min-h-0">
              <FixedHeadTable
                ...
                height="100%"
                emptyText={fetching ? "조회 중..." : "조회 결과가 없습니다. 입고일자를 선택 후 조회하세요."}
